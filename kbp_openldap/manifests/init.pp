...
class kbp_openldap::server inherits openldap::server {
  ...
  define database($directory="", $suffix="", $admins=false, $storage_type="bdb") {
    ...
    file { "/etc/ldap/slapd.db.d/$name.conf":
      ...
      group => "root",
      mode => 640,
      content => template("kbp_openldap/server/database.conf"),
  ...
  openldap::server::schema { "samba":
    source => "puppet://puppet/kbp_openldap/slapd/schema/samba.schema",
  }
  ...
  file { "/etc/ldap/schema/nis.schema":
    source => "puppet://puppet/kbp_openldap/slapd/schema/nis.schema",
    owner => "root",
    group => "root",
    mode => 644,

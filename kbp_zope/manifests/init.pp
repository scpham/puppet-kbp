class kbp_zope inherits zope {
	define site($path, $port, $serveralias=false, $template=false, $ssl=false) {
		...
		#	template => template("kbp_zope/$webserver/site.erb")
		# }
		...
		if tagged(nginx) {
			if $template {
				...
				nginx::site_config { "$domain":
					content => template("kbp_zope/nginx/site.erb"),
					serveralias => $serveralias,
				}
			}
		...
		if tagged(apache) {
			if $template {
				...
				apache::site_config { "$domain":
					address => $ipaddress,
					template => "kbp_zope/apache/site.erb",
					serveralias => $serveralias,
				 }
			}
	...
	define instance($port, $domain=false, $aliases=false, $debugmode=false,
		...
		if tagged(apache) {
			...
			file { "/etc/apache2/conf.d/zope-$name":
				...
				group => "root",
				mode => 644,
				content => template("kbp_zope/apache/proxy.erb"),

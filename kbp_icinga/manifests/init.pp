...
define kbp_icinga::host($conf_dir="false", $use="generic_ha_host", $hostgroups="ha_hosts", $parents="false", $address=$ipaddress, $initialstate="false", $notifications_enabled="false", $event_handler_enabled="false", $flap_detection_enabled="false", $process_perf_data="false", $retain_status_information="false", $retain_nonstatus_information="false", $check_command="false", $check_interval="false", $notification_period="false", $notification_interval="false", $contact_groups="false", $max_check_attempts="false", $register="false") {
	$conf_dir_name = $conf_dir ? {
...
define kbp_icinga::hostgroup($conf_dir="false", $hg_alias, $members="false") {
	$conf_dir_name = $conf_dir ? {
		"false" => "${environment}/${fqdn}",
		default => $conf_dir,
	}
...
define kbp_icinga::contactgroup($conf_dir="false", $customer="generic", $cg_alias) {
	$conf_dir_name = $conf_dir ? {
		"false" => "${environment}/${fqdn}",
		default => $conf_dir,
	}
...
define kbp_icinga::contact($conf_dir="false", $c_alias, $timeperiod="24x7", $notification_type, $contactgroups="false", $contact_data) {
	$conf_dir_name = $conf_dir ? {
		"false" => "${environment}/${fqdn}",
		default => $conf_dir,
	}
...
define kbp_icinga::timeperiod($conf_dir="false", $tp_alias, $monday="false", $tuesday="false", $wednesday="false", $thursday="false", $friday="false", $saturday="false", $sunday="false") {
	$conf_dir_name = $conf_dir ? {
		"false" => "${environment}/${fqdn}",
		default => $conf_dir,
	}
...
define kbp_icinga::configdir($sub=false) {
...
define kbp_icinga::servercommand($conf_dir=false, $commandname=false, $host_argument='-H $HOSTADDRESS$', $argument1=false, $argument2=false, $argument3=false, $nrpe=false, $time_out=false) {
	$conf_dir_name = $conf_dir ? {
		false => "${environment}/${fqdn}",
		default => $conf_dir,
	}
